...
def check_ak_cpp(serial: str, iocs: Dict, opt: Opt, logs: List[Dict]) -> Check:
    ...
    if opt.reboot_for_logcat:
        ...
        if not ok:
            return Check("AK_CPP logcat 검사", False, False, f"reboot failed: {err.strip()}")
        ok, _, err = run_adb(["wait-for-device"], serial, logs, opt.verbose)
        if not ok:
            return Check("AK_CPP logcat 검사", False, False, f"wait-for-device failed: {err.strip()}")
        time.sleep(max(1, opt.logcat_seconds))
    ok, out, err = run_adb(["logcat", "-d"], serial, logs, opt.verbose)
    if not ok:
        return Check("AK_CPP logcat 검사", False, False, f"logcat failed: {err.strip()}")
    hits = [l.strip() for l in out.splitlines() if tag.lower() in l.lower()]
    if not hits:
        return Check("AK_CPP logcat 검사", True, False, f"no '{tag}' match")
    return Check("AK_CPP logcat 검사", True, True, f"{len(hits)} line(s) matched", [Detection("high", "Keenadu debug log tag detected", hits[0][:220])])
...
def check_loader(serial: str, iocs: Dict, logs: List[Dict], verbose: bool) -> Check:
    ...
    if not executed:
        return Check("로더 아티팩트(vndx_10x/.dx) 검사", False, False, "artifact check not executed")
    return Check("로더 아티팩트(vndx_10x/.dx) 검사", True, bool(findings), f"{len(findings)} finding(s)" if findings else "no marker matched", findings)
...
def check_hashes(serial: str, iocs: Dict, installed: Set[str], logs: List[Dict], verbose: bool) -> List[Check]:
    ...
    checks.append(Check("libandroid_runtime.so MD5 검사", executed, bool(lib_find), "matched bad hash" if lib_find else ("no hash match" if executed else "not executed"), lib_find))
    ...
    checks.append(Check("감염 샘플 APK MD5 검사", executed, bool(app_find), "matched bad hash" if app_find else ("no hash match" if executed else "not executed"), app_find))
    return checks
...
def check_static_strings(serial: str, iocs: Dict, logs: List[Dict], verbose: bool) -> Check:
    ...
    if not markers:
        return Check("libandroid_runtime.so 정적 문자열 검사", False, False, "marker list missing")
    ...
    return Check("libandroid_runtime.so 정적 문자열 검사", executed, bool(findings), "matched marker" if findings else ("no marker matched" if executed else "not executed"), findings)
...
def check_boot_snapshots(serial: str, iocs: Dict, opt: Opt, logs: List[Dict]) -> Check:
    ...
    return Check("부팅 후 반복 스냅샷 검사", executed, bool(findings), "marker observed" if findings else ("no marker matched" if executed else "not executed"), findings)


def check_network(serial: str, iocs: Dict, logs: List[Dict], verbose: bool) -> Check:
    domains = iocs.get("c2_endpoints", [])
    if not domains:
        return Check("네트워크 IOC 검사", False, False, "domain list missing")
    findings: List[Detection] = []
    executed = False
    ...
    return Check("네트워크 IOC 검사", executed, bool(findings), "C2 marker observed" if findings else ("no marker matched" if executed else "not executed"), findings)
...
def check_build(serial: str, iocs: Dict, logs: List[Dict], verbose: bool) -> Check:
    ...
    for p in props:
        ...
        if not ok:
            return Check("OTA/빌드 계보 검사", False, False, f"getprop failed: {p} {err.strip()}")
        vals[p] = out.strip()
    findings: List[Detection] = []
    ...
    return Check("OTA/빌드 계보 검사", True, bool(findings), "anomaly detected" if findings else "no anomaly", findings)
...
def check_focus_components(serial: str, iocs: Dict, installed: Set[str], logs: List[Dict], verbose: bool) -> Check:
    ...
    if not iocs.get("focus_component_packages", []):
        return Check("핵심 컴포넌트 검사", False, False, "focus list missing")
    if not focus:
        return Check("핵심 컴포넌트 검사", True, False, "focus package not installed")
    ...
    return Check("핵심 컴포넌트 검사", True, det, "marker found" if det else "no suspicious marker", findings)
...
def check_system_integrity(serial: str, iocs: Dict, installed: Set[str], logs: List[Dict], verbose: bool) -> Check:
    ...
    if not ok:
        return Check("시스템 앱 무결성 검사", False, False, err.strip() or "pm list packages -s -f failed")
    ...
    det = any(f.severity in {"medium", "high"} for f in findings)
    return Check("시스템 앱 무결성 검사", True, det, "anomaly found" if det else "no anomaly", findings)
...
def check_side_loading(serial: str, logs: List[Dict], verbose: bool) -> Check:
    ...
    if not ok:
        return Check("사이드로딩 설정 검사", False, False, err.strip() or "settings read failed")
    if out.strip() == "1":
        return Check("사이드로딩 설정 검사", True, True, "enabled", [Detection("low", "Side-loading enabled", "install_non_market_apps=1")])
    return Check("사이드로딩 설정 검사", True, False, "disabled or unknown")
...
def scan(serial: str, iocs: Dict, opt: Opt, logs: List[Dict]) -> Tuple[List[Check], List[Detection]]:
    ...
    if not ok:
        checks.append(Check("패키지 목록 수집", False, False, msg))
        return checks, findings
    checks.append(Check("패키지 목록 수집", True, False, msg))

    checks.extend([
        check_ak_cpp(serial, iocs, opt, logs),
        check_text_cmd(serial, "Protected Broadcast 검사", "dumpsys activity broadcasts", iocs.get("protected_broadcast_markers", []), logs, opt.verbose, "high"),
        check_text_cmd(serial, "BADBOX Binder 검사", "service list", iocs.get("badbox_binder_markers", []), logs, opt.verbose, "high"),
        Check("감염 샘플 패키지명 검사", True, any(p in installed for p in iocs.get("infected_sample_packages", [])), "matched sample package" if any(p in installed for p in iocs.get("infected_sample_packages", [])) else "no sample package"),
        check_text_cmd(serial, "프로세스/명령 문자열 검사", "ps -A", iocs.get("suspicious_process_markers", []) + iocs.get("suspicious_shell_command_markers", []), logs, opt.verbose, "medium"),
    ...
    while True:
        ...
        try:
            ...
            for d in [x for x in devices if x.state in {"unauthorized", "offline"}]:
                ...
                print("    -> 기기를 연결했을 때 잠금을 풀고 USB 디버깅을 허용해주세요.")
